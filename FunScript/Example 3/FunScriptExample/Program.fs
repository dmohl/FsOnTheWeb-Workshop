...
let addTasksToElement (elementSelector:string) tasks =
    let tasks = tasks 
                |> Array.mapi (fun i task -> "<div class='ui-widget-content draggable'>" + 
                                             task + "</div>" |> box)
    ts.jQuery.Invoke(elementSelector).append tasks |> ignore 

// Create the initial list of to do tasks and tasks that are done.
let populateTasks () =
    let tasksToDo = 
...
let initDragAndDrop () = 
    ...
    // A few helpers from the FunScript.FunctionHelpers module make it a little easier to setup 
    // the Drop callback function. We use one of the overloaded TupledDelegate types to specify
    // the expected signature of the callback function. A standard F# function is then use to all 
    // the element that is being dragged to be appended to the target element. The TupledDelegate
    // type is then turned into an immediate function so that when the event fires the TupledDelegate 
    // function executes immediately. 
    dropSettings.drop <- immediateFn 
                         <| TupledDelegate<ts.Event, ts.DroppableEventUIParam, 
                                           ts.JQuery>(fun e ui -> ui.draggable.appendTo(e.target))
